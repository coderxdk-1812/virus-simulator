...
<style>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* LEFT PANEL */
  .lp{width:300px;min-width:300px;background:var(--panel);
    border-right:1px solid var(--border);display:flex;flex-direction:column;
    padding:18px 20px;gap:14px;overflow-y:auto}

  .stitle{font-family:'Share Tech Mono',monospace;font-size:11px;color:var(--accent);
  ...
  .ctrl-label{font-size:15px;font-weight:700;color:var(--text);letter-spacing:0.3px}
  .cv{font-family:'Share Tech Mono',monospace;font-size:15px;color:var(--accent)}

  /* Range slider — thumb centered on track */
  .range-wrap{
    position:relative;
    width:100%;
    height:20px;          /* tall enough to contain the thumb */
    display:flex;
    align-items:center;
  }
  input[type=range]{
    ...
    left:0;right:0;
    width:100%;
    height:20px;          /* match wrapper so thumb is centered */
  ...
  /* The thumb — vertically centered because input height=20px and thumb=20px */
  input[type=range]::-webkit-slider-thumb{
    ...
    cursor:pointer;
    margin-top:-7px;   /* (5px track - 20px thumb) / 2 = center on track */
  }
  ...
  .sbtn{padding:10px 4px;border:1px solid var(--border);border-radius:6px;
    background:var(--panel2);color:var(--dim);font-size:20px;cursor:pointer;
  ...
  .sbtn.active{border-color:var(--accent);background:rgba(0,212,255,.12);
    color:var(--accent);box-shadow:0 0 10px rgba(0,212,255,.25)}

  .spacer{flex:1;min-height:8px}
  .setup-btn{padding:11px;border:1px solid var(--border);border-radius:6px;
    background:var(--panel2);color:var(--dim);font-family:'Share Tech Mono',monospace;
    font-size:13px;letter-spacing:2px;cursor:pointer;transition:all .2s;width:100%}
  .setup-btn:hover{border-color:var(--accent);color:var(--text)}
  .go-btn{padding:14px;border:2px solid var(--accent);border-radius:8px;
    background:rgba(0,212,255,.08);color:var(--accent);width:100%;
    font-family:'Share Tech Mono',monospace;font-size:17px;letter-spacing:5px;
    cursor:pointer;transition:all .2s;box-shadow:0 0 12px rgba(0,212,255,.25)}
  .go-btn:hover{background:rgba(0,212,255,.2);box-shadow:0 0 24px rgba(0,212,255,.5)}
  ...
  .legend{position:absolute;bottom:12px;left:50%;transform:translateX(-50%);
    ...
    border-radius:8px;padding:8px 22px;z-index:2}
  .li{display:flex;align-items:center;gap:8px;font-size:14px;letter-spacing:1px;font-weight:600}
  .ld{width:11px;height:11px;border-radius:50%;flex-shrink:0}

  /* RIGHT PANEL */
  .rp{width:290px;min-width:290px;background:var(--panel);
    border-left:1px solid var(--border);display:flex;flex-direction:column;
    padding:18px 20px;gap:12px;overflow:hidden}
  .sc{background:var(--panel2);border:1px solid var(--border);border-radius:8px;padding:12px 16px}
  .sl{font-family:'Share Tech Mono',monospace;font-size:10px;color:var(--dim);
    letter-spacing:2px;text-transform:uppercase;margin-bottom:4px}
  .sv{font-family:'Share Tech Mono',monospace;font-size:28px;font-weight:700;line-height:1.1}
  .sbar{height:4px;border-radius:2px;background:var(--border);overflow:hidden;margin-top:7px}
  .sbf{height:100%;border-radius:2px;transition:width .3s}
  #graphCanvas{flex:1;width:100%;border:1px solid var(--border);
    border-radius:8px;background:var(--panel2);display:block}
  .gl{display:grid;grid-template-columns:1fr 1fr;gap:5px 12px;flex-shrink:0}
  .gli{display:flex;align-items:center;gap:7px;font-size:13px;font-weight:600;color:var(--text)}
  .gll{width:20px;height:3px;border-radius:2px;flex-shrink:0}
</style>
</head>
<body>
...
<div class="main">
  <div class="lp">
    ...
    <div class="ctrl">
      ...
      <div class="range-wrap"><input type="range" id="speed" min="1" max="10" value="5"></div>
    </div>
    ...
    <div class="ctrl">
      ...
      <div class="range-wrap"><input type="range" id="population" min="10" max="500" value="150" step="5"></div>
    </div>
    ...
    <div class="ctrl">
      ...
      <div class="range-wrap"><input type="range" id="infectiousness" min="0" max="100" value="65"></div>
    </div>
    ...
    <div class="ctrl">
      ...
      <div class="range-wrap"><input type="range" id="chanceRecover" min="0" max="100" value="25"></div>
    </div>
    ...
    <div class="ctrl">
      ...
      <div class="range-wrap"><input type="range" id="duration" min="1" max="100" value="20"></div>
    </div>
...
class Agent{
  ...
  draw(ctx){
    ...
    if(shape==='person'){
      ...
      ctx.beginPath();ctx.moveTo(-s*.25,s*.45);ctx.lineTo(-s*.5,s*1.45);
...
document.getElementById('goBtn').addEventListener('click',()=>{
  if(agents.length===0)setup();running?stop():start();
});
